...
int main(){
    ...
    while(days > 0){
        ...
        int books_count = days - lib_signup[lib];
        days -= lib_signup[lib];
        int real_books = 0;
        vector<int> boo;
/*
...
*/
         ...
         */
        vector<int> books_can;
        for(int i = 0; i < lib_books[lib]; ++ i){
            if(!book_scanned[lib_has_book[lib][i]]){
                books_can.push_back(lib_has_book[lib][i]);
            }
        }
        if(books_can.size() <= books_count){
            real_books = books_can.size();
            boo = books_can;
            for(auto i: boo){
                score += books_score[i];
                book_scanned[i] = 1;
            ...
            while(books_count >= 0){
                int bookid = rand() % books_can.size();
                int book = books_can[bookid];
                books_can.erase(books_can.begin() + bookid);
                books_count--;
